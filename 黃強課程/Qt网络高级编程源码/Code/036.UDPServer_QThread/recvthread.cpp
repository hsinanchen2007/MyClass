...
RecvThread::RecvThread(QString ip,quint16 port)
{
    this->ip = ip;
    this->port = port;
...
void RecvThread::run()
{
    ...
    //在线程中创建这个对象
      ...
      bool ret= socket->bind(QHostAddress(ip),port ,QUdpSocket::ShareAddress);
      if(ret )
      {
          emit sigPrint("绑定成功！",true);
...
void  RecvThread::slotReadyRead()
{
    ...
    while ( socket->hasPendingDatagrams())
    {
        ...
        //将客户端IP与端口保存到QSet里面
        clients.insert(qMakePair(addr,port));
...
void  RecvThread::send( QByteArray arr)
{
    ...
    {
        //触发发送数据的信号
        emit sigSend( pair.first , pair.second , arr);
    }
}
