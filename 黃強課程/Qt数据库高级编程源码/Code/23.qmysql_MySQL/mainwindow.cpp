...
void MainWindow::on_pushButtonConn_clicked()
{
    ...
    //设置连接参数
    db.setHostName("192.168.16.105");//数据库服务器的IP
    db.setPort(3306);//数据库服务器的端口
    db.setDatabaseName("School");//数据库的名称
    db.setUserName("root"); //连接数据库的用户名
    db.setPassword("root123");//连接数据库的密码
...
void MainWindow::on_pushButtonInsert_clicked()
{
    ...
    //插入数据,自增ID处需要填写DEFAULT
    QString  sql=QString("INSERT INTO Student VALUES(DEFAULT, '%1',%2,%3 )")
            .arg("黄老师").arg(18).arg(99.888);
   ...
   if(ret)
   {
      ...
      // QString sql2= "SELECT @@IDENTITY";
       QString sql2= "SELECT  last_insert_id()";
...
void MainWindow::on_pushButtonInsert_clicked()
{
    ...
    //准备一条SQL语句，使用？当做占位符
    query.prepare("INSERT INTO Student VALUES( DEFAULT,?,?,?)");
    //为占位符赋值
...
void MainWindow::on_pushButtonInsert_clicked()
{
    ...
    query.prepare("INSERT INTO Student VALUES( DEFAULT,:name,:age,:score)");
    //为占位符赋值
...
void MainWindow::on_pushButtonUpdate_clicked()
{
    ...
    QString  sql=QString("UPDATE  Student SET name='%1', "
            ...
            .arg("名称被修改").arg(66).arg(77.77).arg(1);
