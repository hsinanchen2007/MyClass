...
{								  
	printf("sensor 0x%x, temp = %.1f, humi = %d.\n", 
			airQuality->tempHumiData.id, airQuality->tempHumiData.temp, airQuality->tempHumiData.humi);
...
int main(void)
{
	...
	airQuality.tempHumiData.id = 0x1234;
	
	SetTemCof(GetTemCofHmi());

	GetTempHumi(&airQuality.tempHumiData);
